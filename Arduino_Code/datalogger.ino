...
void IRAM_ATTR onTimer() {
    ...
    if (breaking || interruptCounter >= 40) {
        ...
        memcpy((byte *) &logData.canLogData, (byte *) &canData, sizeof(canData));
...
void setup() {
    ...
    */
    ...
    // Delay in Form von blinkender grüner Status-LED um CAN-Modul Zeit zu geben
    for (int i = 0; i <= 5; i++) {
...
void loop() {
    ...
    static uint32_t yellowLedTimer = millis();
    ...
    // Weiterführend wäre es vorstellbar, dass man mit dem Python GUI oder einer Handy-App die gespeicherten Log-Daten
    // direkt auf den PC übermittet. Dies ist ohne weitere Änderungen an der Hardware möglich, da der ESP32 bereits
    // mit Bluetooth und Wifi ausgestattet ist. 
}